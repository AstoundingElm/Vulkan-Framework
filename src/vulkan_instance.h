...
PINLINE VkDebugUtilsMessengerCreateInfoEXT create_debug_info(const void * pnext, VkDebugUtilsMessageSeverityFlagsEXT message_severity, VkDebugUtilsMessageTypeFlagsEXT message_type,
...
PINLINE VkResult create_instance(instance_types * inst_context){
    
    load_global_level_functions(&instance_context);
    ...
    instance_info = create_instance_info(NULL, 0,&app_info, ArraySize(layers), layers, ArraySize(extensions), extensions);
    
    VK_CHECK(vkCreateInstance(&instance_info, 0, &inst_context->instance));
    
    load_instance_level_funcs(&instance_context);
    
    VkDebugUtilsMessengerCreateInfoEXT debug_info = create_debug_info(NULL, VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT, 
                                                                      VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT, vk_debug_callback);
    
    VK_CHECK(vkCreateDebugUtilsMessengerEXT(inst_context->instance, &debug_info, 0, &inst_context->debug_messenger ));
