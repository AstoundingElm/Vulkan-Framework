...
PINLINE VkFenceCreateInfo fence_info(VkFenceCreateFlags flags = 0)
{
...
PINLINE u32 findMemoryType(u32 typeFilter, VkMemoryPropertyFlags properties, physical_device_types * physical_context) {
...
PINLINE VkCommandBuffer beginSingleTimeCommands(logical_device_types * logical_context, command_pool_types * command_context) {
...
PINLINE VkResult load_texture(tex_file_data * load_data, const char * path){
    
    load_data->pixels  = stbi_load(path, &load_texture_data.tex_width, &load_texture_data.tex_height, &load_texture_data.tex_channels, STBI_rgb_alpha);
    if(!load_data->pixels){
...
PINLINE void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory, logical_device_types * logical_context) {
    ...
    if (vkAllocateMemory(logical_context->logical_device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS) {
        printf("failed to allocated buffer memry");
    }
    
    vkBindBufferMemory(logical_context->logical_device, buffer, bufferMemory, 0);
};

PINLINE void createImage(u32 width, u32 height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, logical_device_types * logical_context) {
...
PINLINE void endSingleTimeCommands(VkCommandBuffer commandBuffer, queue_types * queue_context, logical_device_types * logical_context,command_pool_types * command_context
                                   ) {
...
PINLINE void transitionImageLayout(VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout) {
...
PINLINE void copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands(&logical_device_context, &command_context);
...
PINLINE VkResult create_image_view(logical_device_types * logical_context, Image * internal_create_image){
    
    VkImageViewCreateInfo view_info = create_image_view_info(NULL,0,  internal_create_image->image, VK_IMAGE_VIEW_TYPE_2D, VK_FORMAT_R8G8B8A8_UNORM );
    VK_CHECK(vkCreateImageView(logical_context->logical_device, &view_info, 0, &internal_create_image->view));
    return VK_SUCCESS;
};

PINLINE Image * material_image(logical_device_types * logical_context, tex_file_data * load_data,  asset_type_id ed){
    Image * image = 0;
    if(image_count < MAX_IMAGES){
        
        asset_type_id id = ASSET_SPRITE_FONT_ATLAS;//get_asset(id);
        /*
        if(id == ASSET_SPRITE_DEFAULT){
            
            VK_CHECK(load_texture(load_data,"/home/petermiller/Desktop/4coder/hello/src/download.jpeg"));
            ...
            if(id == ASSET_SPRITE_WHITE){
                */
        VK_CHECK(load_texture(load_data,"/home/petermiller/Desktop/4coder/hello/src/font_atlas.png"));
        
        //}
        //}
        
        VkDeviceSize image_size = load_data->tex_width * load_data->tex_height * 4;
        ...
        if(image->image && image->memory){
            ...
            transitionImageLayout(image->image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
            
            copyBufferToImage(staging_buffer.buffer, image->image, (u32)load_data->tex_width, (u32)load_data->tex_height);
            
            transitionImageLayout(image->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
PINLINE u32 vk_get_memory_type_index(
...
PINLINE Image vk_allocate_image(
                                VkDevice device,
                                VkPhysicalDevice gpu,
                                uint32_t width,
                                uint32_t height,
...
PINLINE Buffer vk_allocate_buffer(
                                  VkDevice device,
                                  VkPhysicalDevice gpu,
                                  uint32_t size,
...
PINLINE VkSamplerCreateInfo create_sampler_info(const void * pnext, VkSamplerCreateFlags flags, VkFilter mag_filter, VkFilter min_filter
