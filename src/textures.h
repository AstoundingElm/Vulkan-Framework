...
/*PINLINE VkResult copy_to_buffer(const  void * data, u32 size){
    assert(staging_buffer.size >= size);
    
    if(staging_buffer.data){
...
PINLINE VkResult create_image_view(logical_device_types * logical_context){
    
    VkImageViewCreateInfo view_info = create_image_view_info(NULL,0,  tex_image.image, VK_IMAGE_VIEW_TYPE_2D, VK_FORMAT_R8G8B8A8_UNORM );
    VK_CHECK(vkCreateImageView(logical_context->logical_device, &view_info, 0, &tex_image.view));
    return VK_SUCCESS;
};
