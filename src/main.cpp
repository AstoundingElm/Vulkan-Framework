...
int main(){
    ...
    KTRACE("a test mesage");*/
    input_state.screen_size = {1000, 700};
    Game_State game_state = {};
    init_xwindow(&xcb_context, input_state.screen_size );
    ...
    global_image = material_image(&logical_device_context, &load_texture_data, ASSET_SPRITE_WHITE);
    ...
    while(vulkan_running){
        ...
        update_ui(&ui);
        update_game(&game_state,&input_state ,(f32)delta,&ui );
        
        clock_gettime(CLOCK_MONOTONIC, &current_tick_count);
        //u64 elapsed_ticks = current_tick_count - last_tick_count;
        VK_CHECK(render_loop(&logical_device_context, &semaphore_context, &command_context, &swapchain_context, &q_context, &renderpass_context, &framebuffer_context, &pipeline_context, &screen_context, &game_state, &ui));
