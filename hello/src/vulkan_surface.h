...
PINLINE VkResult create_linux_surface(instance_types * instance_context, surface_types * surface_context){
    ...
    VK_CHECK(vkCreateXcbSurfaceKHR(instance_context->instance, &surface_create_info, instance_context->allocator, &surface_context->surface));
    return VK_SUCCESS;
    
}
/*
PINLINE VkResult create_surface(instance_types * instance_context, physical_device_types * device_types, queue_types * queue_context, surface_types * surface_context){
    ...
    VK_CHECK(vkGetPhysicalDeviceSurfaceSupportKHR(device_types->physical_device_best_device, queue_context->queue_family_best_queue_index, surface_context->surface, &surface_support));
    
	vkGetPhysicalDeviceSurfaceCapabilitiesKHR
    (device_types->physical_device_best_device, surface_context->surface,&surface_context->surface_capabilities);
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(device_types->physical_device_best_device, surface_context->surface, &surface_context->surface_format_count, NULL);
    
    vkGetPhysicalDeviceSurfaceFormatsKHR(device_types->physical_device_best_device, surface_context->surface, &surface_context->surface_format_count, surface_context->surface_formats);
    
    u32 present_mode_count;
    
    vkGetPhysicalDeviceSurfacePresentModesKHR(device_types->physical_device_best_device, surface_context->surface, &present_mode_count, NULL);
    
    VkPresentModeKHR present_modes[present_mode_count];
    vkGetPhysicalDeviceSurfacePresentModesKHR(device_types->physical_device_best_device, surface_context->surface, &present_mode_count, present_modes);
