...
PINLINE VkResult create_graphics_pipeline(pipeline_types * pipeline_context, renderpass_types * render_context, instance_types * instance_context, logical_device_types * logical_context,
                                          descriptor_set_types * set_context){
    
    
    VkPipelineLayoutCreateInfo layout_info = create_pipeline_layout_info(1, &set_context->set_layout, 0, NULL);
    VK_CHECK(vkCreatePipelineLayout(logical_context->logical_device, &layout_info, 0, &pipeline_context->pipeline_layout));
    ...
    VkPipelineRasterizationStateCreateInfo rasterization_state = create_rasterization_state_info(VK_FALSE, VK_FALSE, VK_POLYGON_MODE_FILL, VK_CULL_MODE_BACK_BIT, VK_FRONT_FACE_CLOCKWISE,VK_FALSE,0, 0,0,  1.0f);
    ...
    VkShaderModuleCreateInfo vert_module = create_shader_module_info(NULL, 0, shader_data.shader_loaded ? shader_data.vert_size :0, 
                                                                     shader_data.shader_loaded ? (const u32 *)shader_data.vert_code : NULL);
    ...
    VK_CHECK(vkCreateShaderModule(logical_context->logical_device, &vert_module, instance_context->allocator, &vert_shader_module));
    
    VK_CHECK(vkCreateShaderModule(logical_context->logical_device, &frag_module, instance_context->allocator, &frag_shader_module));
