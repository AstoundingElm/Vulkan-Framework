...
int main() {
    ...
    vkGetDeviceQueue(vk_context.logical_device, queue_family_best_queue_index, 0, &graphics_queue);
    char single_queue = 1;
    ...
    VK_CHECK(vkCreateXcbSurfaceKHR(vk_context.instance, &surface_create_info, vk_context.allocator, &vk_context.surface));
  ...
  VkSurfaceCapabilitiesKHR surface_capabilities;
	vkGetPhysicalDeviceSurfaceCapabilitiesKHR
		(*physical_device_best_device, vk_context.surface,&surface_capabilities);
	...
	actual_extent.width= reply->width;
	actual_extent.height= reply->height;
	if(surface_capabilities.currentExtent.width!=wind_w||surface_capabilities.currentExtent.height!=wind_h){
		extent_suitable=0;
		printf("actual extent size doesn't match framebuffers, resizing...\n");
		actual_extent.width=wind_w>surface_capabilities.maxImageExtent.width? surface_capabilities.maxImageExtent.width:wind_w;
		actual_extent.width=wind_w<surface_capabilities.minImageExtent.width?surface_capabilities.minImageExtent.width:wind_w;
		actual_extent.height=wind_h>surface_capabilities.maxImageExtent.height?surface_capabilities.maxImageExtent.height:wind_h;
		actual_extent.height=wind_h<surface_capabilities.minImageExtent.height?surface_capabilities.minImageExtent.height:wind_h;
	}
...
surface_formats[0].colorSpace, extent_suitable ? surface_capabilities.currentExtent : actual_extent,   1, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, 
single_queue ? VK_SHARING_MODE_EXCLUSIVE : VK_SHARING_MODE_CONCURRENT, single_queue ? 0 : 2,  single_queue ? NULL : queue_family_indices, 
surface_capabilities.currentTransform, VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR, mailbox_mode_supported ? VK_PRESENT_MODE_MAILBOX_KHR : VK_PRESENT_MODE_FIFO_KHR, 
VK_TRUE, VK_NULL_HANDLE);

    VkSwapchainKHR swap;
    VK_CHECK(vkCreateSwapchainKHR(vk_context.logical_device, &swap_info, vk_context.allocator, &swap));

    uint32_t swap_image_count;
    vkGetSwapchainImagesKHR(vk_context.logical_device, swap, &swap_image_count, NULL);
    VkImage swap_images[swap_image_count];
    vkGetSwapchainImagesKHR(vk_context.logical_device, swap, &swap_image_count, swap_images);
    ...
    VK_CHECK(vkCreateShaderModule(vk_context.logical_device, &vert_module_create_info, vk_context.allocator, &vert_shader_module));
VK_CHECK(vkCreateShaderModule(vk_context.logical_device, &frag_module_create_info, vk_context.allocator, &frag_shader_module));
...
for(uint32_t i = 0; i < swap_image_count; i++){
    ...
    VK_CHECK(vkCreateFramebuffer(vk_context.logical_device, &(framebuffer_create_info[i]), vk_context.allocator, &(frame_buffers[i])));
};

VkCommandPoolCreateInfo command_pool_create_info = create_command_pool_info( queue_family_best_queue_index);

VkCommandPool command_pool;
VK_CHECK(vkCreateCommandPool(vk_context.logical_device, &command_pool_create_info, NULL, &command_pool));
...
for(uint32_t i = 0; i < max_frames; i++){
    ...
    VK_CHECK(vkCreateSemaphore(vk_context.logical_device, &semaphore_create_info, vk_context.allocator, &(semaphore_image_available[i])));
   VK_CHECK( vkCreateSemaphore(vk_context.logical_device, &semaphore_create_info, vk_context.allocator, &(semaphore_render_final[i])));
    VK_CHECK(vkCreateFence(vk_context.logical_device, &fence_create_info, vk_context.allocator, &(fence[i])));
...
for(uint32_t i = 0; i < swap_image_count; i++){
  ...
  VK_CHECK(vkWaitForFences(vk_context.logical_device, 1, &(fence[current_frame]), VK_TRUE, UINT64_MAX));
        uint32_t image_index = 0;
        VK_CHECK(vkAcquireNextImageKHR(vk_context.logical_device, swap, UINT64_MAX, semaphore_image_available[current_frame], VK_NULL_HANDLE, &image_index));
        
        if(image_fence[image_index]!= VK_NULL_HANDLE){

            VK_CHECK(vkWaitForFences(vk_context.logical_device, 1, &(image_fence[image_index]), VK_TRUE, UINT64_MAX));
        ...
        submit_info.pSignalSemaphores = &(signal_semaphore[0]);

       VK_CHECK(vkResetFences(vk_context.logical_device, 1, &(fence[current_frame])));
        vkQueueSubmit(graphics_queue, 1, &submit_info, fence[current_frame]);
